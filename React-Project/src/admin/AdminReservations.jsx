...
const TABLE_OPTIONS = [
  { number: "T1", name: "Table 1", capacity: "1–4 Guests" },
  { number: "T2", name: "Table 2", capacity: "1–4 Guests" },
  { number: "T3", name: "Table 3", capacity: "1–4 Guests" },
  { number: "T4", name: "Table 4", capacity: "1–4 Guests" },
  { number: "T5", name: "Table 5", capacity: "4–8 Guests" },
  { number: "T6", name: "Table 6", capacity: "4–8 Guests" },
  { number: "T7", name: "Table 7", capacity: "4–10 Guests" },
  { number: "T8", name: "Table 8", capacity: "2 Guests" },
  { number: "T9", name: "Table 9", capacity: "2–8 Guests" },
  { number: "T10", name: "Table 10", capacity: "2–8 Guests" },
];
...
const getOccasion = (reservation) => {
  return reservation?.occasion || "General Dining";
};

const getStatus = (reservation) => {
  return String(reservation?.status || "PENDING").toUpperCase();
};
...
export default function AdminReservations() {
  ...
  useEffect(() => {
    ...
    fetchReservations();
  }, [authLoading, user, navigate, fetchReservations]);
  ...
  const filteredReservations = useMemo(() => {
    ...
    return reservations.filter((reservation) => {
      ...
      const searchableText = [
        ...
        getTableType(reservation),
        getOccasion(reservation),
  ...
  const openApproveModal = (reservation) => {
    ...
    setTableNumber(reservation?.assignedTable || "");
        ...
        previous.map((reservation) =>
          reservation._id === selectedReservation._id
            ? updatedReservation || {
                ...reservation,
                status: "CONFIRMED",
                assignedTable: tableNumber.trim(),
                adminNote: adminNote.trim(),
              }
            : reservation
        )
  ...
  const isTableAlreadyAssigned = (table) => {
    ...
    return reservations.some((reservation) => {
      ...
      if (getStatus(reservation) !== "CONFIRMED") {
        return false;
      }

      const sameDate =
        String(reservation.date || "").slice(0, 10) ===
        String(selectedReservation.date || "").slice(0, 10);

      const sameTime =
        String(reservation.time || "") ===
        String(selectedReservation.time || "");

      const sameTable =
        String(reservation.assignedTable || "").toLowerCase() ===
        String(table.number || "").toLowerCase();

      return sameDate && sameTime && sameTable;
    });
  };
    ...
    <div className="admin-reservations-page">
      ...
      <header className="admin-reservations-header">
        <div>
          ...
          <div className="admin-title-row">
            <div>
              <p className="admin-eyebrow">SWAAD & SPICE</p>

              <h1>Reservation Management</h1>

              <p className="admin-subtitle">
                Manage table bookings, approve requests and handle
                reservation status.
              </p>
            </div>
          </div>
      ...
      <section className="reservation-stats">
        <div className="reservation-stat-card">
          <div className="reservation-stat-icon">📋</div>
              ...
              >
                ...
                <div className="reservation-card-top">
                  <div className="reservation-customer">
                    ...
                    <div>
                      <h2>{getCustomerName(reservation)}</h2>
                ...
                <div className="reservation-details-grid">
                  ...
                  <div className="reservation-detail">
                    ...
                    <strong>
                      👥 {getGuests(reservation)} Guests
                    </strong>
                  </div>

                  <div className="reservation-detail">
                    <span className="detail-label">Table Type</span>
                    <strong>🍽️ {getTableType(reservation)}</strong>
                  </div>

                  <div className="reservation-detail">
                    <span className="detail-label">Occasion</span>
                    <strong>🎉 {getOccasion(reservation)}</strong>
                  </div>

                  <div className="reservation-detail">
                    <span className="detail-label">Price</span>
                    <strong>
                      ₹{Number(reservation.tablePrice || 0).toLocaleString(
                        "en-IN"
                      )}
                    </strong>
                  </div>
                ...
                <div className="reservation-contact">
                  <div>
                    <span>📞</span>
                    <span>{getCustomerPhone(reservation)}</span>
                  </div>

                  <div>
                    <span>✉️</span>
                    <span>{getCustomerEmail(reservation)}</span>
                  </div>
                </div>
                ...
                {status === "REJECTED" && (
                  <div className="rejected-info">
                    <span>✕</span>
                    <p>This reservation has been rejected.</p>
                  </div>
                )}
          ...
          >
            ...
            <div className="modal-reservation-summary">
              ...
              <div>
                ...
                <p>
                  {getGuests(selectedReservation)} Guests •{" "}
                  {getTableType(selectedReservation)}
                </p>
              </div>
            </div>
              ...
              <>
                ...
                {tableNumber && (
                  <div className="table-selected-message">
                    ✓ {tableNumber} selected for this reservation
                  </div>
                )}
              </>
            )}
            ...
            <div className="reservation-modal-actions">
              ...
              {modalMode === "approve" ? (
                <button
                  ...
                  disabled={actionLoading || !tableNumber}
                >
                  {actionLoading
                    ? "Confirming..."
                    : "✓ Confirm Reservation"}
